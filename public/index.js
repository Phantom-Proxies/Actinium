...
class crypts {
  static encode( str ) {
    return encodeURIComponent( [ ...str ].map( ( c, i ) => i % 2 ? String
      .fromCharCode( c.charCodeAt() ^ 2 ) : c ).join( "" ) );
  }
  static decode( str ) {
    if ( str.endsWith( "/" ) ) str = str.slice( 0, -1 );
    return decodeURIComponent( [ ...str ].map( ( c, i ) => i % 2 ? String
      .fromCharCode( c.charCodeAt() ^ 2 ) : c ).join( "" ) );
  }
}
...
if ( "serviceWorker" in navigator ) {
  ...
  navigator.serviceWorker.register( proxy.file, {
      scope: proxy.config.prefix
    } )
    .then( reg => {
      console.log( "ServiceWorker registered:", reg.scope );
      form.addEventListener( "submit", event => {
        event.preventDefault();
        location.href = proxy.config.prefix + crypts.encode( search(
          address.value ) );
      } );
    } )
