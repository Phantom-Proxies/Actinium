      ...
      function loadInActiveTab(input) {
        ...
        iframe.onload = () => {
          console.log("Iframe loaded:", destinationUrl);
          let loadedTitle = destinationUrl.split('/')[2] || 'Page Loaded';
          try {
            if (iframe.contentWindow?.document?.title) {
              loadedTitle = iframe.contentWindow.document.title ||
                loadedTitle;
            }
          } catch (err) {
          ...
          updateTabDetails(activeTabId, destinationUrl,
            loadedTitle);
        };
      ...
      function startMessageCycling(msgElement) {
        ...
        function cycle() {
          const currentWelcomeScreen = document.getElementById(elementId);
          if (!currentWelcomeScreen || !currentWelcomeScreen.contains(
              msgElement)) {
